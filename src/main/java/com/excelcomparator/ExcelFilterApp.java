...
public class ExcelFilterApp extends JFrame {

    private CardLayout cardLayout;
    private JPanel mainPanel;
    private File file1, file2;
    private JTextField file1Path, file2Path;
    private JRadioButton f1toF2Radio, f2toF1Radio;

    private JPanel attributeMappingPanel, reportMappingPanel, productMappingPanel;
    private JComboBox<String> file1AttributeCombo;
    private JComboBox<String> file2ReportCombo, file2ProductCombo;
    ...
    public ExcelFilterApp() {
        setTitle("Excel Filter App");
        setSize(900, 700);
        ...
        add(mainPanel);
        cardLayout.show(mainPanel, LOAD_PANEL);
    }
    ...
    private JPanel createLoadPanel() {
        ...
        f2toF1Radio = new JRadioButton("Use Excel 2 to filter Excel 1");
        ...
        gbc.gridy = 4; gbc.gridx = 1; gbc.gridwidth = 1; gbc.anchor = GridBagConstraints.CENTER;
    ...
    private void showMappingPanel() {
        if (file1 == null || file2 == null) {
            JOptionPane.showMessageDialog(this, "Please select both Excel files.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        try {
            ...
            JOptionPane.showMessageDialog(this, "Error reading Excel files: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void addDynamicMappingRow(JPanel parent, String label, List<String> headers) {
        ...
        parent.add(row, parent.getComponentCount() - 1); // Add before the "Add" button
        parent.revalidate();
    }
    ...
    private List<String> getDynamicMappings(JPanel parent) {
        ...
        for (Component comp : parent.getComponents()) {
            // The rows we add are JPanels themselves
            if (comp instanceof JPanel) {
                JPanel rowPanel = (JPanel) comp;
                // We are looking for a JComboBox inside the row panel
                for(Component C : rowPanel.getComponents()){
                    if (C instanceof JComboBox) {
                         mappings.add((String) ((JComboBox<?>) C).getSelectedItem());
                    }
                }
            }
        }
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ExcelFilterApp app = new ExcelFilterApp();
            app.setVisible(true);
        });
    }
}
